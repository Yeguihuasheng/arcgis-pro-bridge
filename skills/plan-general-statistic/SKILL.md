...
name_en: 通用面积统计
name_zh: 通用面积统计
description: 通用面积统计 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 通用面积统计 or similar operations.
description_en: 通用面积统计 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 通用面积统计 or similar operations.
description_zh: 业务技能：通用面积统计（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「通用面积统计」或同类操作时使用。
