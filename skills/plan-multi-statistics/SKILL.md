...
name_en: 智能汇总统计
name_zh: 智能汇总统计
description: 智能汇总统计 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 智能汇总统计 or similar operations.
description_en: 智能汇总统计 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 智能汇总统计 or similar operations.
description_zh: 业务技能：智能汇总统计（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「智能汇总统计」或同类操作时使用。
