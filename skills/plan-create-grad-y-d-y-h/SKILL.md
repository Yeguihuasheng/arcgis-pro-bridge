...
name_en: 生成分级用地用海编码名称
name_zh: 生成分级用地用海编码名称
description: 生成分级用地用海编码名称 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 生成分级用地用海编码名称 or similar operations.
description_en: 生成分级用地用海编码名称 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 生成分级用地用海编码名称 or similar operations.
description_zh: 业务技能：生成分级用地用海编码名称（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「生成分级用地用海编码名称」或同类操作时使用。
...
生成分级用地用海编码名称 【生成分级用地用海编码名称】
所属分组：规划应用 ｜ 执行方式：🟢 插件直执行
一、简介
按输入数据生成新成果。本条属于「规划应用」组。
二、参数介绍
