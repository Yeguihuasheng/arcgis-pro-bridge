---
name: plan-statistics-s-d-l
name_en: 三调_统计三大类
name_zh: 三调_统计三大类
description: 三调_统计三大类 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 三调_统计三大类 or similar operations.
description_en: 三调_统计三大类 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 三调_统计三大类 or similar operations.
description_zh: 业务技能：三调_统计三大类（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「三调_统计三大类」或同类操作时使用。
...
**原窗体参数项**（提取自界面定义，作问询参照）：选择三调图层： ／ 选择地块图层： ／ 面积类型： ／ 面积单位： ／ 保留小数位数： ／ 统计指标
