---
name: plan-s-d2-y-d-y-h
name_en: 三调转用地用海
name_zh: 三调转用地用海
description: 三调转用地用海 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 三调转用地用海 or similar operations.
description_en: 三调转用地用海 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 三调转用地用海 or similar operations.
description_zh: 业务技能：三调转用地用海（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「三调转用地用海」或同类操作时使用。
...
**原窗体参数项**（提取自界面定义，作问询参照）：输入三调图层： ／ 输出用地用海名称字段： ／ 版本：

**界面固定选项**：一对多的转为【待细分】 ｜ 一对多的转为一级类 ｜ 全部转为一级类
...
**源码**：完整随附于本技能目录——`source/SD2YDYH.xaml`、`source/SD2YDYH.xaml.cs`

**算法**：三调（DLMC 名称）→ 用地用海（目标字段），靠**新旧两版映射 Excel**（resources/ 内两份）：
1. CheckData 校验图层与 DLMC 字段；
2. 转换类型三选一：`通用` / `待细分转一级类` / `全部转一级类`（对应映射表里的 3 个 sheet）；
3. 版本二选一：`旧版`→三调用地名称_to_用地用海用地名称.xlsx；`新版`→…_新版.xlsx；
4. 复制映射表到工程 Home 文件夹 → AttributeMapper(in_data, "DLMC", map_field, sheet) 逐行替换；
5. 删除临时 xlsx。
