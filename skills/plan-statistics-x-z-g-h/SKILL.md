...
name_en: 用地用海现状规划指标汇总
name_zh: 用地用海现状规划指标汇总
description: 用地用海现状规划指标汇总 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 用地用海现状规划指标汇总 or similar operations.
description_en: 用地用海现状规划指标汇总 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 用地用海现状规划指标汇总 or similar operations.
description_zh: 业务技能：用地用海现状规划指标汇总（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「用地用海现状规划指标汇总」或同类操作时使用。
...
## 执行路径
✅ 首选本插件：**用地工具箱▶用地指标汇总（本插件）** —— 打开对应按钮窗体，按下方参数填写后执行。
