...
name_en: 生成道路交叉口
name_zh: 生成道路交叉口
description: 生成道路交叉口 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 生成道路交叉口 or similar operations.
description_en: 生成道路交叉口 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 生成道路交叉口 or similar operations.
description_zh: 业务技能：生成道路交叉口（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「生成道路交叉口」或同类操作时使用。
...
## 执行路径
✅ 首选本插件：**道路工具箱▶生成道路交叉口（本插件）** —— 打开对应按钮窗体，按下方参数填写后执行。
