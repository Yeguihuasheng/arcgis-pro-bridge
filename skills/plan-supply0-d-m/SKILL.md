...
name_en: 用地代码后补充0
name_zh: 用地代码后补充0
description: 用地代码后补充0 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 用地代码后补充0 or similar operations.
description_en: 用地代码后补充0 (规划应用). Runs inside ArcGIS Pro via the YghsBridge MCP channel. Use when the user asks for 用地代码后补充0 or similar operations.
description_zh: 业务技能：用地代码后补充0（规划应用）。经 YghsBridge MCP 在 ArcGIS Pro 内执行。用户提到「用地代码后补充0」或同类操作时使用。
